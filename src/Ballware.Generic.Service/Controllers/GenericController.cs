...
namespace Ballware.Generic.Service.Controllers
{
    ...
    public class GenericController : Controller
    {
            ...
            try
            {
                ...
                foreach (var file in files)
                {
                    if (file != null)
                    {
                        ...
                        var job = await MetaClient.CreateJobForTenantBehalfOfUserAsync(tenantId, currentUserId, jobPayload);
        ...
        )
        {
            ...
            {
                var export = await GenericProvider.ExportAsync(tenant, metaData, identifier, claims, queryParams);
                var exportEntry = await MetaClient.CreateExportForTenantBehalfOfUserAsync(tenantId, currentUserId);
                ...
                await StorageClient.UploadFileForOwnerAsync("export", new[] { new FileParameter(new MemoryStream(export.Data), $"{exportEntry.Id}{MimeTypeMap.GetExtension(export.MediaType)}", export.MediaType) });
                
                await MetaClient.SaveExportBehalfOfUserAsync(tenantId, currentUserId, exportEntry);

                return Ok(exportEntry.Id.ToString());
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, ex);
        ...
        )
        {
            ...
            var export = await MetaClient.FetchExportByIdForTenantAsync(tenantId, id);
