...
public interface ITenantGenericProvider
{
    Task<IEnumerable<T>> AllAsync<T>(Metadata.Tenant tenant, Entity entity, string identifier,
        Guid userId, IDictionary<string, object> claims) where T : class;

    Task<IEnumerable<T>> QueryAsync<T>(Metadata.Tenant tenant, Entity entity, string identifier,
        Guid userId, IDictionary<string, object> claims, IDictionary<string, object> queryParams) where T : class;

    Task<long> CountAsync(Metadata.Tenant tenant, Entity entity, string identifier,
        Guid userId, IDictionary<string, object> claims, IDictionary<string, object> queryParams);

    Task<T?> ByIdAsync<T>(Metadata.Tenant tenant, Entity entity, string identifier,
        Guid userId, IDictionary<string, object> claims, Guid id) where T : class;
    ...
    Task SaveAsync(Metadata.Tenant tenant, Entity entity, string identifier,
        Guid userId, IDictionary<string, object> claims, IDictionary<string, object> value);
    ...
    Task<bool> StateAllowedAsync(Metadata.Tenant tenant, Entity entity, Guid id, int currentState, Guid userId, IDictionary<string, object> claims, IEnumerable<string> rights);
